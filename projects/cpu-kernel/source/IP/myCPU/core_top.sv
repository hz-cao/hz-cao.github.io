...
// difftest — simulation-only DPI co-verification hooks. These modules exist
// only in the Verilator testbench (as DPI stubs that call NEMU); they have
// no hardware definition and MUST be excluded from FPGA synthesis. The
// SYNTHESIS macro is auto-defined by Vivado during synth_design; Verilator
// does not define it, so the sim build still gets the hooks.
`ifndef SYNTHESIS
