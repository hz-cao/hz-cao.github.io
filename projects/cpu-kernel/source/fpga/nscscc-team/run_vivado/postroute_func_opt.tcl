...
set input_dcp [file join $script_dir "build/func/routed_safe.dcp"]
set output_dcp [file join $script_dir "build/func/postroute_optimized.dcp"]
...
puts "FUNC_POSTROUTE_WNS=$wns"
puts "FUNC_POSTROUTE_WHS=$whs"
write_checkpoint -force $output_dcp
if {$wns < 0.0 || $whs < 0.0} {
    puts "FUNC_POSTROUTE_TIMING_FAILED"
    exit 2
}

write_debug_probes -force [file join $bit_dir "soc_top_func.ltx"]
write_bitstream -force [file join $bit_dir "soc_top_func.bit"]
puts "FUNC_POSTROUTE_BIT_OUTPUT=[file join $bit_dir soc_top_func.bit]"
exit 0
